...
public class HelperUtil {
    ...
    (
        ...
        BaseDialogFragment dialog = BaseDialogFragment.newInstance(_title, _message);
        if (dialog == null) {
            return;
        }
        if (_onClickRunable != null) {
            dialog.setOnClickListener(_onClickRunable);
        }
        if (_activity != null && _finishActivity == true) {
            dialog.setFinishActivity(_activity);
        }

        try {
            dialog.show(_activity.getFragmentManager(), "dialog");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
